...
router.post("/getAllTrackInfo", verifyToken, async (req, res) => {
  if (typeof req.body.trackID == "string") {
    //Action here
    res.json(await tracks.getAllInfo(req.body.trackID, req.user.id))
  } else {
    res.json({
      success: false,
      error: "Invalid api call"
...
router.post("/editTrack", verifyToken, async (req, res) => {
  if (typeof req.body.trackID == "string" && typeof req.body.newData == "object") {
    //Action here
...
router.post("/deleteTrack", verifyToken, async (req, res) => {
  if (typeof req.body.trackID == "string") {
    //Action here
    res.json(await tracks.delete(req.body.trackID, req.user.id))
  } else {
    res.json({
      success: false,
      error: "Invalid api call"
